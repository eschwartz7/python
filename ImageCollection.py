...
NUM_IMAGES = 150  # number of images to grab
...
def grab_next_image_by_trigger(nodemap, cam):
    ...
    try:
        ...
        if CHOSEN_TRIGGER == TriggerType.SOFTWARE:
            # Wait n seconds to trigger image capture:
            time.sleep(2)
...
def run_single_camera(cam):
    ...
    try:
        ...
        node_iGammaEnable_bool.SetValue(True)
        node_iGamma_float = PySpin.CFloatPtr(nodemap.GetNode("Gamma"))
        node_iGamma_float.SetValue(1)

        # Disable White Balancing
        node_balancewhiteauto_mode = PySpin.CEnumerationPtr(nodemap.GetNode("BalanceWhiteAuto"))
        node_balancewhiteauto_mode_off = node_balancewhiteauto_mode.GetEntryByName("Off")
        node_balancewhiteauto_mode.SetIntValue(node_balancewhiteauto_mode_off.GetValue())

        # Set Black Level Offset to 0%
        node_iBlackLevel_float = PySpin.CFloatPtr(nodemap.GetNode("BlackLevel"))
        node_iBlackLevel_float.SetValue(0)

        # Allows us to change Gain
        # node_iGain_float = PySpin.CFloatPtr(nodemap.GetNode("Gain"))
        # node_iGain_float.SetValue(gain_value)

        #Disables Automatic Exposure
        node_exposure_auto_mode = PySpin.CEnumerationPtr(nodemap.GetNode("ExposureAuto"))
        node_exposure_auto_mode_off = node_exposure_auto_mode.GetEntryByName("Off")
        node_exposure_auto_mode.SetIntValue(node_exposure_auto_mode_off.GetValue())
        ...
        exposure = 15000
        print(exposure)
        #Allows us to change exposure time
        node_iExposureTime_float = PySpin.CFloatPtr(nodemap.GetNode("ExposureTime"))
        node_iExposureTime_float.SetValue(exposure)
        print(exposure)
